...
function bookCategory(category){
    ...
    for (let book of filterByCategory[0].books){
        ...
        let info = `Title: ${book.title}, isbn: ${book.isbn}, released: ${book.release_date}, pages: ${book.num_of_pages}, price: ${book.price}`
        li.textContent = info
...
function printPrice(category) {
    let filterByCategory = bookList.filter((book)=> (book.name === category));
    let books = filterByCategory[0].books;
    let { minPrice, maxPrice } = getPrice(books);
